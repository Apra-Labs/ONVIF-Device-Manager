# ODM Credentials UI — Plan Execution
...
1. Run the project build step (e.g. `msbuild odm.sln`) first, then run the full test suite. Both must pass.
2. Confirm all prior tasks in the group work correctly
3. Update progress.json with test results and issues found
4. `git push origin feat/credentials-ui` — code must be on origin before PM reviews
5. STOP — do not continue. Report status so the PM can review.

## Branch Hygiene
- Before creating a branch: `git fetch origin && git checkout origin/development`
- Before pushing a PR or at PM's request: `git fetch origin && git rebase origin/development`, rerun tests after rebase
